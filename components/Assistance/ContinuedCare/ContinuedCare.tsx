...
const ContinuedCare: React.FC<ContinuedCareProps> = ({ assistanceData }) => {
    const section = assistanceData?.continuedCareSection
        ...
        <section>
            ...
            <div className="h-max bg-[#4AA6B1] px-4 md:px-14 flex flex-col items-center justify-center">
                <header className="mx-auto max-w-6xl px-6 py-10 text-white flex flex-col gap-8 justify-center items-center">
                    ...
                    >
                        {section?.title || 'Continued Care for Growing Children'}
                    </h2>
                    <p className="text-center text-balance text-2xl relative font-medium">
                        {section?.description || 'Our connection with families grows stronger with time. For children graduating from our programs, we offer additional opportunities for continued growth and joyful exploration.'}
                    </p>
                </header>
                <div className='max-w-[1000px] grid md:grid-cols-3 md:grid-rows-1 grid-cols-1 grid-row-3 items-center justify-center text-white text-center gap-16'>
                    {section?.careFeatures?.map((feature, index) => (
