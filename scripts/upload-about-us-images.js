...
async function uploadAboutUsImages() {
  console.log('🚀 Uploading About Us page images to Sanity...\n')

  try {
    // About Us specific images based on component analysis
    console.log('📸 Uploading About Us images...')

    // Hero Section Background
    const heroImage = await uploadImage('About_hero_image.jpg', 'About Us Hero Background Image')

    // Vision & Mission Icons
    const visionIcon = await uploadImage('eyes.svg', 'Vision Icon - Eyes Symbol')
    const missionIcon = await uploadImage('hand-heart.svg', 'Mission Icon - Hand Heart Symbol')

    // CEO Section Image
    const ceoImage = await uploadImage('CEO.jpg', 'Chandrika Bharath - CEO & Founder Photo')

    // Let's Connect Section Images (shared with home but referenced in about us)
    ...
    // FAQ Section Images
    const cloudIcon = await uploadImage('cloud.svg', 'Cloud Icon for FAQ Section')

    console.log('\n📄 Updating About Us document with uploaded images...')

    // Check if aboutUs document exists
    const existingDoc = await client.fetch('*[_type == "aboutUs"][0]{_id}')

    if (!existingDoc) {
      console.log('❌ No About Us document found. Creating new one...')
    }
    ...
    console.log('✅ Successfully updated About Us document with images!')
    console.log(`📋 Document ID: ${result._id}`)

    console.log('\n🎉 About Us images upload completed successfully!')
    console.log('\n📊 Summary:')
    console.log(`✅ Hero Background: ${heroImage ? 'Uploaded' : 'Skipped'}`)
    console.log(`✅ Vision Icon: ${visionIcon ? 'Uploaded' : 'Skipped'}`)
    console.log(`✅ Mission Icon: ${missionIcon ? 'Uploaded' : 'Skipped'}`)
    console.log(`✅ CEO Photo: ${ceoImage ? 'Uploaded' : 'Skipped'}`)
    ...
    console.log(`✅ Cloud Icon: ${cloudIcon ? 'Uploaded' : 'Skipped'}`)
